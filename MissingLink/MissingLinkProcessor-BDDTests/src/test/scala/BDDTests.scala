...
import ch.dersalvador.MissingLinkProcessor.model._
import ch.dersalvador.MissingLinkProcessor.LocalPropertiesLocator
...
class BDDTests extends FlatSpec with Matchers {
    ...
    "Application path should exist " {
        val ara = new ARAWebserviceWrapper(missinLinkProcessorModell)
        val b: Boolean  = ara.checkExistance("Applications/DerSalvador/DER/abs/13.3.0.dersalvador.12");
        b shouldBe(true)
        0
    }
